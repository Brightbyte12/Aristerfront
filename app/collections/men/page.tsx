...
export default function MenCollectionsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const menProducts = allProducts.filter((p: any) => p.gender?.toLowerCase() === "men");
        const productsData = menProducts.map((p: any) => {
  ...
  // Add helper to get total stock and available sizes from variants
    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
        <nav className="text-xs sm:text-sm text-gray-600">
          ...
          <span className="mx-2">/</span>
          <span className="text-darkGreen">Men</span>
        </nav>
      </div>
        ...
        <div className="flex gap-4 lg:gap-8">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
              <div>
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-light">Men's Collections</h1>
                    ...
                    <CardContent className="p-0">
                        ...
                        <NextImage
                            src={
                              hoveredProduct === product.id && product.colorImages && product.colorImages.length > 0 && product.colorImages[0].images && product.colorImages[0].images.length > 1
                                ...
                                ? product.images[hoveredImageIndex[product.id] || 0]?.url || product.images[0].url
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {getAvailableSizes(product)?.slice(0, 3).map((size: string, sizeIndex: number) => (
                            <Badge
                              key={sizeIndex}
                              variant="outline"
                          ...
                          {getAvailableSizes(product) && getAvailableSizes(product).length > 3 && (
                            <Badge
                              variant="outline"
                              className="text-responsive-xs border-mocha text-darkGreen"
                            >
                              +{getAvailableSizes(product).length - 3}
                            </Badge>
                          )}
                        </div>
                      </div>
