...
import { Heart, Filter, X, ChevronDown, Grid3X3, List, Check, Star, ShoppingBag } from "lucide-react";
...
export default function CollectionsPage() {
  ...
  // Add state to track which product was just added
  const [addedProductId, setAddedProductId] = useState<string | null>(null);

  const [collectionsHeading, setCollectionsHeading] = useState({ text: "Explore Our Diverse Collections", fontColor: "#FFFDEB", bgColor: "#A27B5C" });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const invalidProducts = allProducts.filter((p: Product) => !p.material);
        if (invalidProducts.length > 0) {
          console.warn(
            "Products with undefined material detected:",
            invalidProducts.map((p) => ({ id: p.id, name: p.name }))
          );
        }
        const productsData = allProducts.map((p: any) => {
          // Extract unique sizes from variants if sizes is empty
  ...
  useEffect(() => {
    axios.get("/api/settings/public").then(res => {
      setCollectionsHeading(res.data.allCollectionsHeading || { text: "Explore Our Diverse Collections", fontColor: "#FFFDEB", bgColor: "#A27B5C" });
  ...
  return (
    <div className="min-h-screen bg-cream">
      <div
        ...
        style={{ backgroundColor: collectionsHeading.bgColor, color: collectionsHeading.fontColor }}
      >
        {collectionsHeading.text}
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
        <nav className="text-xs sm:text-sm text-gray-600">
          ...
          <span className="text-darkGreen">Collections</span>
        </nav>
      </div>

      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex gap-4 lg:gap-8">
          <div className="hidden lg:block w-64 xl:w-72 flex suite-400 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
              <div>
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-light">All Collections</h1>
